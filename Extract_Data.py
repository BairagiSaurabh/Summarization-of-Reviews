...
def main_file(webpage, page_number, pages_to_extract):
    ...
    def get_splitted_reviews(df):
        ...
        for i, j in enumerate(df["Review"].values):  # for each review
            ...
            raws = [(df["Review"].values[i]) for h in range(len(review_split_))]
...
try:
    ...
    fig = px.bar(st.session_state.dfinal,x = "Score",y = "Aspect",title = "Sentiments for Aspects",color="Score",orientation = 'h')
    st.plotly_chart(fig)

    def show_senti(data_,senti):
        data_show = data_[data_["Sentiment"] == str(senti)]
        data_show_imp = data_show[["Review", "Sentiment"]]
        data_display = data_show_imp.drop_duplicates(subset=["Review"])
